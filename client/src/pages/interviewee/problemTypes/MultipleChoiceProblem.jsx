...
export default function MultipleChoiceProblem({ problem, onExit }) {
  ...
  // Persist state
  useEffect(() => {
    localStorage.setItem(storageKey, JSON.stringify({ selected, submitted, isCorrect, attempt }))
  }, [selected, submitted, isCorrect, attempt])
                ...
                <form onSubmit={e => { e.preventDefault(); setShowSubmitDialog(true) }}>
                  <div className="space-y-4 mb-6 mt-2">
                    {problem.options.map((opt, idx) => (
                      <label key={idx} className={`flex items-center gap-3 p-3 rounded border cursor-pointer transition ${selected === idx ? 'border-primary bg-primary/10' : 'border-border hover:bg-muted/40'}`}>
                        <input
                          type="radio"
                          name="option"
                          value={idx}
                  ...
                  {!submitted && (
                    <Button type="submit" className="w-full" disabled={selected === null || loading}>
